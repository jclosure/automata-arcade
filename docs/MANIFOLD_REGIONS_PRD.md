...
1. **Boundary Protocol B — asymmetry**: When a flat cell just outside the region has a manifold cell as a neighbor, should that flat cell's neighbor count include the manifold cell? (Currently: yes, by default, since the flat cell uses its own standard neighbor list which just happens to include a key that's inside the region.) This creates the asymmetric "membrane leaks outward" effect. Is this the desired default?

2. **Resizing a region**: When a region is resized, all cells that were inside and are now outside are instantly returned to flat topology. Their alive state is preserved. Is there any edge case where a topologically "wrapped" cell becomes unreachable after a resize?

3. **Script Kernel API surface**: What should the Kernel API for manifold regions look like? Proposed:
   ```javascript
   kernel.manifoldRegions            // array of region objects
   kernel.manifoldRegions[0].shape   // 'torus'
   kernel.manifoldRegions[0].christoffelAt(x, y)  // Float32Array
   kernel.manifoldRegions[0].neighborsOf(x, y)    // [[x,y], ...]
   kernel.createManifoldRegion({rect, shape})
   kernel.removeManifoldRegion(id)
   ```

4. **Lenia compatibility**: Lenia uses a continuous kernel with float weights. The manifold neighbor map currently stores keys (binary membership). For Lenia in a manifold region, we need to store `Map<cellKey, Array<{key, weight}>>`. Should this be a separate `leniaNeighborMap` or should the base `neighborMap` be weight-aware from the start?

---

*This document is the authoritative reference for the Manifold Regions feature. Implementation should not begin on any phase until this document is agreed upon. All ambiguities should be resolved here before they are resolved in code.*
